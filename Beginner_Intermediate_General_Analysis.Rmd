---
title: "Beginner_Intermediate_General_Analysis"
author: "Ashraf Alsinglawi"
date: "2024-09-19"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Load Required Libraries

```{r Load Libraries, include=FALSE}
library(tidyverse)
library(lubridate)
```

## Load the Dataset
```{r Loading Data, include=FALSE}
data <- read_csv("healthcare_dataset.csv")
```
## Data structure and summary
```{r Data Summary, echo=FALSE}
glimpse(data)
```
As we see, there are 15 columns(variables) with 10000 rows. All seems to be with the correct data types (will check later)

What is needed here:
1. Change variables names.
2. Check dates data types
3. Think about new features to extract and add for deep analysis


```{r Data Summary, echo=FALSE}
summary(data)
```
As we see, We have 10 character variables. Gender, Blood Type, Admission Type and Test Results could be categorical in addition to Age if we want to analyze age categories. 3 numerical Age, Billing Amount and Room Number, the latter is not very informative, so we consider drop it. 2 Date variables.

## Including Plots


Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.
